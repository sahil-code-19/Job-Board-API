...
async def create_application(
    ...
    cover_letter: str = Form(default=None),
):
    ALLOWED_TYPES = [
        "application/pdf",
        "application/msword",
    ...
    if file.content_type not in ALLOWED_TYPES:
        raise HTTPException(
            status_code=400, detail="Only PDF or Word documents allowed"
        )
    ...
    application = await apply_job(
        db, file_location, current_user.id, job_id, cover_letter
    )
